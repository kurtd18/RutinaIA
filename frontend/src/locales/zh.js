...
export default {
  ...
  "This browser doesn't support passkeys — you can still use RutinaIA locally on this device.": '此浏览器不支持通行密钥——你仍可在本设备上本地使用 RutinaIA。',
  ...
  'to install RutinaIA as a full-screen app.': '即可将 RutinaIA 安装为全屏应用。',
  ...
  'A small file a friend imports into their own RutinaIA — routines only, none of your workouts or weigh-ins.': '一个小文件，朋友可以导入到自己的 RutinaIA——只有训练日，不包含你的训练记录或体重。',
  ...
  'Your server': '你的服务器',
  'Synced with your RutinaIA server.': '已与你的 RutinaIA 服务器同步。',
  'Disconnect': '断开连接',
  ...
  'Sync this device to your own self-hosted RutinaIA instead.': '改为将此设备与你自己的自托管 RutinaIA 同步。',
  'Pair the mobile app': '配对手机应用',
  'Connect the RutinaIA app on your phone to this account.': '将手机上的 RutinaIA 应用连接到此账户。',
  'Could not generate a code': '无法生成配对码',
  'On the RutinaIA app, choose “Connect to my server”, then enter this address and the code below. It expires in 5 minutes.': '在 RutinaIA 应用中选择“连接到我的服务器”，然后输入此地址和下面的配对码。5 分钟后失效。',
  'Enter your server address and the code': '输入你的服务器地址和配对码',
  'Connected': '已连接',
  'Could not connect': '无法连接',
  'Open Settings → “Pair the mobile app” on the RutinaIA site you’re already signed into, then enter its address and the code shown there.': '在你已登录的 RutinaIA 网站上，打开设置 →“配对手机应用”，然后输入其地址和那里显示的配对码。',
  'Server address (e.g. gym.example.com)': '服务器地址（例如 gym.example.com）',
  'Pairing code': '配对码',
  'Connecting…': '连接中…',
  'Connect': '连接',
  'How do you want to use RutinaIA?': '你想如何使用 RutinaIA？',
  'Use on this device': '在此设备上使用',
  'Local keeps everything on this phone. Connecting syncs to your own RutinaIA server instead — you can switch later in Settings.': '本地模式会把所有内容保留在这部手机上。连接则会改为与你自己的 RutinaIA 服务器同步——之后可以在设置中更改。',
